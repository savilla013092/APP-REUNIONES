    ...
    try {
        ...
        try {
            ...
            if (apiError?.message?.includes("not found") || apiError?.status === 404) {
                throw new Error("El modelo de Gemini no esta disponible. Verifica tu API key en Google AI Studio.");
            }
            if (apiError?.message?.includes("API_KEY") || apiError?.status === 401 || apiError?.status === 403) {
                throw new Error("API Key de Gemini invalida o sin permisos. Verifica tu configuracion en .env.local");
            }
            if (apiError?.message?.includes("SAFETY")) {
                throw new Error("El contenido fue bloqueado por filtros de seguridad de Gemini. Intenta con un contenido diferente.");
            }
            throw apiError;
        }
        ...
        if (!parsed.introduction && !parsed.development) {
            ...
            throw new Error(
                "La respuesta de Gemini no contiene los campos esperados. Intenta de nuevo."
            );
        }
        ...
        if (errorMsg.includes("blocked") || errorMsg.includes("safety")) {
            throw new Error("Contenido bloqueado por filtros de seguridad. Modifica el texto e intenta de nuevo.");
        }

        // Show the actual error for debugging
        throw new Error("Error de Gemini: " + (error?.message || "Error desconocido. Revisa la consola para mas detalles."));
    }
};
