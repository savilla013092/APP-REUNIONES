...
export const generateActaWord = async (acta: Acta) => {
        ...
        rows: [
            ...
            new TableRow({
                children: [
                    ...
                    new TableCell({ children: [new Paragraph(`${acta.meetingInfo.startTime} - ${acta.meetingInfo.endTime || 'N/A'}`)] }),
                ],
            }),
            new TableRow({
                children: [
                    new TableCell({ children: [new Paragraph({ children: [new TextRun({ text: "Lugar:", bold: true })] })] }),
    ...
    // Introduction
    sections.push(
        new Paragraph({ text: "1. PREÁMBULO", heading: HeadingLevel.HEADING_3, spacing: { after: 120 } }),
        new Paragraph({ text: acta.generatedContent?.introduction || "", spacing: { after: 300 } })
    );

    // Development
    sections.push(
        new Paragraph({ text: "2. DESARROLLO DE LA REUNIÓN", heading: HeadingLevel.HEADING_3, spacing: { after: 120 } }),
        new Paragraph({ text: acta.generatedContent?.development || "", spacing: { after: 300 } })
    );

    // Agreements
    if (acta.generatedContent?.agreements.length) {
        sections.push(new Paragraph({ text: "3. ACUERDOS Y DECISIONES", heading: HeadingLevel.HEADING_3, spacing: { after: 120 } }));
        acta.generatedContent.agreements.forEach((agreement, i) => {
            sections.push(new Paragraph({ text: `${i + 1}. ${agreement}`, bullet: { level: 0 }, spacing: { after: 100 } }));
        });
    ...
    if (acta.generatedContent?.commitments.length) {
        sections.push(new Paragraph({ text: "4. COMPROMISOS", heading: HeadingLevel.HEADING_3, spacing: { after: 120 } }));

        const commitmentRows = [
            new TableRow({
                children: [
                    new TableCell({ children: [new Paragraph({ children: [new TextRun({ text: "Compromiso", bold: true })] })] }),
                    new TableCell({ children: [new Paragraph({ children: [new TextRun({ text: "Responsable", bold: true })] })] }),
                    new TableCell({ children: [new Paragraph({ children: [new TextRun({ text: "Vencimiento", bold: true })] })] }),
                ],
            }),
        ];
            ...
            commitmentRows.push(
                new TableRow({
                    children: [
                        ...
                        new TableCell({ children: [new Paragraph(c.dueDate ? format((c.dueDate as any).toDate(), "dd/MM/yyyy") : "-")] }),
                    ],
                })
            );
        });

        sections.push(new Table({ width: { size: 100, type: WidthType.PERCENTAGE }, rows: commitmentRows }), new Paragraph({ text: "", spacing: { before: 400 } }));
    }

    // Signatures
    sections.push(new Paragraph({ text: "FIRMAS ELECTRÓNICAS", heading: HeadingLevel.HEADING_3, spacing: { after: 200 } }));
    ...
    for (const attendee of signedAttendees) {
        try {
            ...
            sections.push(
                ...
                new Paragraph({
                    children: [
                        new TextRun({ text: attendee.name, bold: true }),
                    ],
                }),
