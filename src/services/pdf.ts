...
export const generateActaPDF = async (acta: Acta) => {
    ...
    if (acta.generatedContent?.commitments.length) {
        ...
        autoTable(doc, {
            ...
            body: acta.generatedContent.commitments.map(c => [
                c.description,
                c.responsible,
                c.dueDate ? (c.dueDate as any).toDate ? format((c.dueDate as any).toDate(), "dd/MM/yyyy") : c.dueDate.toString() : "-"
            ]),
            theme: "grid",
        });
    ...
    doc.save(`Acta_${acta.meetingInfo.title.replace(/\s+/g, '_')}_${format(new Date(), 'yyyyMMdd')}.pdf`);
};
