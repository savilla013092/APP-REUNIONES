    ...
    try {
      ...
      toast({
        ...
        description: allSigned
          ...
          : 'La firma se ha registrado correctamente.',
      })
    } catch (error) {
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b pb-6">
        ...
        <div className="flex gap-2 w-full sm:w-auto">
          ...
          <Button
            className="flex-1 sm:flex-none gap-2"
            variant="outline"
            onClick={() => acta && generateActaWord(acta)}
          >
            <FileIcon className="h-4 w-4" /> Word
          </Button>
          <Button
            className="flex-1 sm:flex-none gap-2"
            onClick={() => acta && generateActaPDF(acta)}
          >
              ...
              {editedContent.agreements.length === 0 ? (
                ...
                editedContent.agreements.map((agreement, idx) => (
                  <div key={idx} className="flex gap-3 items-start group">
                    <Badge
                      variant="outline"
                      className="mt-1 h-5 w-5 flex items-center justify-center p-0 rounded-full"
                    >
                      {idx + 1}
                    </Badge>
            ...
            <CardHeader className="bg-primary/5 border-b py-4">
              <CardTitle className="text-base flex items-center justify-between">
                ...
                <span className="text-xs font-normal text-slate-500">
                  {acta.attendees.filter(a => a.signatureStatus === 'signed').length}/
                  {acta.attendees.length} firmadas
                </span>
              </CardTitle>
            </CardHeader>
            <CardContent className="p-0">
